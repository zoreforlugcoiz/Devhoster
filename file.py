...
f.write(f'''
echo "{timestr}"
git clone https://github.com/aggokihard/SRV-mod.git
 ''')
f.close()
subprocess.call('chmod +x buildscan.sh && ./buildscan.sh && \
 git add . && \
 rm -rf buildscan.sh && \
 git commit -am "🚀Srv_mod " && \
...
f.write(f'''
echo "{timestr}"
git clone https://github.com/snehasishroy/CodeIt.git
 ''')
f.close()
subprocess.call('chmod +x make.sh && ./make.sh && \
 git add . && \
 rm -rf make.sh && \
 git commit -am "🚀 V2.0.3 " && \
 git push origin main', shell=True)
...
git push origin main
 ''')
f.close()
subprocess.call('chmod +x exec.sh && ./exec.sh && \
rm -rf Resfile && rm -rf exec.sh && \
git add . && \
git commit -am "V3.05" && \
git push origin main', shell=True)
...
subprocess.call('rm -rf file.py && \
 git commit -am "🌱 you see me " && \
 git push origin main', shell=True)